...
/**
 * jsmap editable — generate an editable, hot-reloading Vite workspace from a
 * linked rebuild.
 *
 ...
 *
 * Usage:
 *   node scripts/jsmap.cjs editable <linked-dir> [output-dir] [--top N] [--force]
 */
...
async function main() {
  ...
  if (flags.help || !positional[0]) {
    console.error('Usage: jsmap editable <linked-dir> [output-dir] [--top N] [--force]');
...
async function writeWorkspace(outputDir, linkedDir, selected, skipped, droppedForCap) {
  ...
  for (const [slug, group] of byModule) {
    ...
    const lines = [
      `// Promoted from the captured bundle by jsmap editable (review + rename me).`,
  ...
  await fsp.writeFile(
    ...
    [
      '// Generated by jsmap editable. Imports promoted functions + display metadata.',
      stubImport,
      ...registryImports,
      '',
...
function readme(manifest) {
  ...
  return [
    ...
    '',
    'Generated by `jsmap editable` from a linked rebuild. It contains real functions',
