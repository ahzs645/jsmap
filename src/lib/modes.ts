...
export const MODE_CONFIG: Record<AppMode, ModeConfig> = {
  mapper: {
    ...
    heroDescription:
      'Batch source maps, remote bundles, site snapshots, and pasted inputs through a worker-backed pipeline with real mapping lookups and built-in bundle cleanup.',
    heroBadges: ['Queue driven', 'Worker isolated', 'Bundle cleanup'],
  ...
  deobfuscator: {
    ...
    pasteDescription:
      'Paste raw source map JSON or JavaScript to recover sources, or use built-in structural deobfuscation when no map is available.',
    urlDescription:
